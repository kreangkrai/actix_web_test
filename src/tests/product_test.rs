...
#[actix_web::test]
    async fn test_gets_ok() {
        let req = test::TestRequest::default()
            ...
            .to_http_request();
        let resp = get_products(req).await.unwrap();
...
mod integration_tests {
    ...
    #[actix_web::test]
    async fn test_get() {
        let app = test::init_service(App::new().route("/", web::get().to(get_products))).await;                                  
        let req = test::TestRequest::default()
            .insert_header(ContentType::plaintext())
            .to_request();
    ...
    #[actix_web::test]
    async fn test_post() {
        let app = test::init_service(App::new().service(web::resource("/").to(get_products))).await;
